...
BEGIN
	...
	--DECLARE @ProjectNo NVARCHAR(100)='S32A1305700';
	--Prepare temp data
	SELECT * INTO #TMP_DDV
	...
	SELECT * INTO #TMP_BP
	FROM BOM_Plan BP
	WHERE BP.Plan_Name=@PlanName AND BP.Project_No=Project_No
	...
	SELECT @ProjectNo,@PlanName,CS.Parts_Name,CS.Spec,VAR_TYPE,VAR_VALUE,SUM(CS.LHS),SUM(CS.RHS),SUM(CS.PCE),SUM(CS.LHS+CS.RHS+CS.PCE)*SUM(CS.UNIT_WEIGHT),CS.Process_ID,CS.COLOR,@REVERSION,@CRTIME
